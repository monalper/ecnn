// frontend/src/components/video/VideoCommentForm.jsx
...
const VideoCommentForm = ({ videoId, parentCommentId = null, parentAuthorName = null, parentAuthorUsername = null, onCommentAdded, onCancel, isModal = false }) => {
  const { user } = useAuth();
  const [formData, setFormData] = useState({
    content: parentAuthorName ? `@${parentAuthorUsername || parentAuthorName} ` : ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const commentData = {
        articleSlug: videoId, // Video ID'sini articleSlug olarak kullanıyoruz
        content: formData.content.trim(),
        parentCommentId
      };

      console.log('Video yorumu gönderiliyor:', commentData);
      const response = await api.post('/comments', commentData);
      console.log('Video yorumu gönderme yanıtı:', response.data);
      
      setSuccess(response.data.message);
      
      // Formu temizle
      setFormData({ content: parentAuthorName ? `@${parentAuthorUsername || parentAuthorName} ` : '' });
      ...
      console.error('Video yorumu gönderme hatası:', error);
      ...
      <div className={isModal ? "" : ""}>
        <div className="text-center py-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              to="/login"
              className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-full text-white bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
            >
              Giriş Yap
            </Link>
            <Link
    ...
    <div className={isModal ? "" : ""}>
      {error && (
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
          <p className="text-sm text-blue-600 dark:text-blue-400 font-medium">{error}</p>
        </div>
      )}
            ...
            {parentAuthorName && (
              <div className="mb-2">
                <span className="text-xs text-blue-500 font-medium">
                  Yanıt: @{parentAuthorUsername || parentAuthorName}
                </span>
              </div>
            )}
