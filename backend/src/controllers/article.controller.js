...
exports.getArticleBySlug = async (req, res, next) => {
  ...
  try {
    ...
    // Eğer kullanıcı admin değilse ve makale 'published' veya 'unlisted' değilse, gösterme
    if (!isAdminUser && article.status !== 'published' && article.status !== 'unlisted') {
...
exports.getHighlightedArticles = async (req, res, next) => {
  try {
    ...
    // Sadece published makaleleri getir (unlisted makaleler öne çıkarılamaz)
    const scanParams = {
      TableName: ARTICLES_TABLE,
...
exports.searchArticles = async (req, res, next) => {
  try {
    ...
    const cachedResults = articleCache.get(cacheKey);
    if (cachedResults) {
      return res.json(cachedResults);
    }
    
    // Tüm published makaleleri getir
    ...
    // Başlık, açıklama ve içerikte arama yap
    const results = (Items || [])
      .filter(article => {
      ...
      .map(article => ({
        ...
        slug: article.slug,
        title: article.title,
        excerpt: article.description || (article.content ? article.content.substring(0, 150) + '...' : ''),
        coverImage: article.coverImage,
        publishedAt: article.publishedAt
      }));
    
    // Cache'e kaydet
    articleCache.set(cacheKey, results, 5 * 60 * 1000); // 5 dakika
