const { docClient, USERS_TABLE, ARTICLES_TABLE } = require('../config/aws.config');
...
// Kullanıcının makaleyi kaydetme/kaldırma
exports.toggleSavedArticle = async (req, res, next) => {
  const userId = req.user.userId; // verifyToken middleware'den
  const { slug } = req.params;
  const { action } = req.body; // 'save' veya 'unsave'

  if (!action || (action !== 'save' && action !== 'unsave')) {
    return res.status(400).json({ message: 'Geçerli bir işlem belirtin (save veya unsave).' });
  }

  try {
    // Önce makalenin var olup olmadığını kontrol et
    const articleParams = { TableName: ARTICLES_TABLE, Key: { slug } };
    const { Item: article } = await docClient.send(new GetCommand(articleParams));

    if (!article) {
      return res.status(404).json({ message: 'Makale bulunamadı.' });
    }

    const updatedUser = await UserModel.toggleUserSavedArticle(userId, slug, action);
    
    res.status(200).json({
      message: action === 'save' ? 'Makale kaydedildi.' : 'Makale kayıtlardan kaldırıldı.',
    ...
    if (error.message === 'User already saved this article') {
      return res.status(400).json({ message: 'Bu makale zaten kaydedilmiş.' });
    }
    if (error.message === 'User has not saved this article') {
      return res.status(400).json({ message: 'Bu makale kayıtlı değil.' });
    }
    console.error("Toggle Saved Article Error:", error);
    next(error);
  }
};
...
exports.getSavedArticles = async (req, res, next) => {
  ...
  try {
    const savedArticleSlugs = await UserModel.getUserSavedArticles(userId);
    
    if (!savedArticleSlugs || savedArticleSlugs.length === 0) {
      return res.status(200).json([]);
    }

    // Kaydedilen makaleleri getir
    const articles = await Promise.all(
      savedArticleSlugs.map(async (slug) => {
        try {
          const params = { TableName: ARTICLES_TABLE, Key: { slug } };
    ...
    // Null olmayan makaleleri filtrele ve tarihe göre sırala
    const validArticles = articles.filter(article => article !== null);
    const sortedArticles = validArticles.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
...
// Kullanıcının bir makaleyi kaydedip kaydetmediğini kontrol et
exports.checkArticleSaved = async (req, res, next) => {
  const userId = req.user.userId;
  const { slug } = req.params;

  try {
    const isSaved = await UserModel.hasUserSavedArticle(userId, slug);
    res.status(200).json({ isSaved });
  } catch (error) {
    console.error("Check Article Saved Error:", error);
    next(error);
  }
};